 ...
 *
 * Gnahatum carried its own copy of the Gemini model ids, pinned to dated preview
 * builds that Google has since retired. lib/llm.ts already listed one of those
 * exact ids in its DEPRECATED_MODELS map — Gnahatum simply never consulted it.
 *
 * These keep Gnahatum's ids aligned with the list the working LMS chat uses.
 */
...
test("gnahatum offers at least one Gemini model", () => {
  expect(araGoogle.length).toBeGreaterThan(0);
});

test("every gnahatum Gemini id is one the LMS also uses", () => {
