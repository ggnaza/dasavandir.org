...
// GET /api/admin/space-members?userId=… — the space ids a user belongs to (current org).
export async function GET(req: Request) {
  ...
  const { data, error } = await admin.from("space_members").select("space_id").eq("user_id", userId);
...
// POST /api/admin/space-members — add a user to a space (in the current org).
export async function POST(req: Request) {
  ...
  // The space must belong to the current org.
  ...
  const { error } = await admin
    .from("space_members")
    .upsert({ space_id: spaceId, user_id: userId, role: "learner" }, { onConflict: "space_id,user_id", ignoreDuplicates: true });
  if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
  return Response.json({ ok: true });
}

// DELETE /api/admin/space-members?userId=…&spaceId=… — remove a user from a space.
export async function DELETE(req: Request) {
  ...
  const { error } = await admin.from("space_members").delete().eq("user_id", userId).eq("space_id", spaceId);
  if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
  return Response.json({ ok: true });
}
