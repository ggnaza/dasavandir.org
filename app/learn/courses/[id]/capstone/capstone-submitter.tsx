...
export function CapstoneSubmitter({
  capstone,
  existingSubmission,
  courseId,
  allLessonsCompleted,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (file) {
      ...
      const path = `capstones/${user!.id}/${Date.now()}-${safeName}`;
    ...
    const res = await fetch("/api/capstones/submit", {
      method: "POST",
    ...
    return (
      <div className="space-y-4">
        <div className={`rounded-xl p-4 text-sm font-medium ${
          ...
          {isApproved && "✓ Your capstone has been reviewed and approved."}
          {isReturned && "↩ Your capstone has been returned with feedback."}
        ...
        {(isApproved || isReturned) && sub.final_feedback && (
          <div className="bg-white border rounded-xl p-5 space-y-3">
            ...
            {(sub.final_feedback as unknown as FeedbackItem[]).map((item, i) => (
  ...
  if (!allLessonsCompleted) {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-sm text-amber-800">
        <p className="font-semibold mb-1">Complete all lessons first</p>
        <p>You need to finish every lesson in this course before submitting your capstone project.</p>
      </div>
    );
  }
      ...
      {capstone.rubric?.length > 0 && (
        <div className="bg-gray-50 border rounded-xl p-4">
          ...
          <ul className="space-y-1">
            {capstone.rubric.map((item: any, i: number) => (
          ...
          <p className="text-xs text-gray-400 mt-2">
            Total: {capstone.rubric.reduce((s: number, r: any) => s + r.max_points, 0)} points
          </p>
        </div>
      )}
