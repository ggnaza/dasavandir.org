...
export default async function AdminQuizPage({
  ...
  const [{ data: lesson }, { data: quiz }, { data: bankCount }] = await Promise.all([
    ...
    admin.from("question_bank").select("id", { count: "exact", head: true }).eq("course_id", params.id),
  ]);
  ...
  return (
    <div className="max-w-2xl">
      ...
      <QuizEditor lessonId={params.lessonId} courseId={params.id} existing={quiz} bankQuestionCount={(bankCount as any)?.count ?? 0} />
    </div>
  );
}
