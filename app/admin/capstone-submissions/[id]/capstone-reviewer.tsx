...
export function CapstoneReviewer({ submission, rubric }: { submission: any; rubric: RubricItem[] }) {
  ...
  async function handleAction(action: "approve" | "return") {
    setSubmitting(true);
    await fetch("/api/capstones/review", {
      method: "POST",
    ...
    setSubmitting(false);
    router.push("/admin/capstone-submissions");
    router.refresh();
  }
      ...
      {ai ? (
        <div className="bg-white border rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-brand-600">{totalScore} / {totalPossible} pts</span>
          </div>
          {ai.overall_comment && (
                  ...
                  <div className="flex items-center gap-1">
                    {isDone ? (
                      ...
                      <input
                        ...
                        onChange={(e) => setScores({ ...scores, [item.criterion]: Number(e.target.value) })}
